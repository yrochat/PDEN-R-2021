...
ColorBrewer, nommée d'après une de ses auteurs, Cnythia Brewer, est une librairie de couleurs précalculées qui s'accordent bien ([voir son site web](https://colorbrewer2.org/)).
...
### Palettes disponibles et daltonisme
...
Les palettes adaptées aux daltonien·ne·s
...
```{r}
ggplot(diam, aes(x = carat, y = price, fill = clarity, color = clarity)) + 
  geom_point(shape = 21) +
  scale_color_brewer() +
  scale_fill_brewer()
```
...
```{r}
ggplot(diam, aes(x = carat, y = price, label = 1:1000)) + 
  geom_text()
```
